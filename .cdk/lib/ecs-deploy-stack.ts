...
import { IVpc, IRoute53, IAcm, ISecretsManager, IContainer, ICloudWatchAlarm, IAutoscaling, ILoadBalancer, ITask } from '../config'
...
export class EcsDeployStack extends Stack {
  constructor(scope: Construct, id: string, props: EcsDeployStackProps) {
    ...
    const certificate = this.getAcmCertificate(protocol, hostedZone, fullDomainName, props.acm?.arn)
    ...
    const cluster = props.clusterArn
      ? ecs.Cluster.fromClusterArn(this, 'EcsCluster', props.clusterArn)
    ...
    new route53.ARecord(this, 'WebServerDomainToLoadBalancer', {
      recordName: props.route53.hostname,
      zone: hostedZone,
    ...
    ecsService.metricCpuUtilization().createAlarm(this, 'AlarmCpuUtilization', {
      ...
      alarmDescription: `Alarming CPU Utilization higher than ${props.cloudWatchAlarm?.cpu.alarmThreshold || 60} for service ${props.appName}`,
      comparisonOperator: cloudwatch.ComparisonOperator.GREATER_THAN_OR_EQUAL_TO_THRESHOLD,
    })

    // ALARM MEMORY UTILIZATION
    ecsService.metricMemoryUtilization().createAlarm(this, 'AlarmMemoryUtilization', {
      ...
      alarmDescription: `Alarming Memory Utilization higher than ${props.cloudWatchAlarm?.memory.alarmThreshold || 75} for service ${props.appName}`,
      comparisonOperator: cloudwatch.ComparisonOperator.GREATER_THAN_OR_EQUAL_TO_THRESHOLD,
    })

    // ALARM ECS RUNNING TASK
    ecsService.metric('RunningTaskCount').createAlarm(this, 'AlarmRunningTaskCount', {
      alarmName: `AlarmAlarm${props.stackName}RunningTaskCount`,
      threshold: props.autoscaling?.minCapacity || 1,
      evaluationPeriods: 1,
      alarmDescription: `Alarming Tasks running bellow than ${props.autoscaling?.minCapacity || 1} for service ${props.appName}`,
  ...
  private generateSecretList = (secrets: ISecretsManager[]) => {
    ...
    secrets.forEach((secret, index) => {
      const secretsManager = secretsmanager.Secret.fromSecretCompleteArn(this, `Secret${index}`, secret.arn)
  ...
  private getAcmCertificate = (protocol: 'HTTP' | 'HTTPS', hostedZone?: route53.IHostedZone, domainName?: string, certificateArn?: string) => {
